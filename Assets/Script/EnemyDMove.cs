...
public class EnemyDMove : MonoBehaviour {
	...
	void Update()
	{
		if (frame > 3) {
			...
			if (player == null) {
				player = GameObject.Find ("Player_HP").transform;
			}
			...
			if (isEnabled == true) {
				...
				playerDirection.y = 0;
				transform.rotation = Quaternion.Slerp (transform.rotation, Quaternion.LookRotation (playerDirection - transform.position), Time.time * 0.1f);
